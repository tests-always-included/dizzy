...
function determineArgs(callable) {
    ...
    match = str.match(/^(?:async[\s\w]+)?function[^(]*\(([\s\w,]*?)\)/);
    ...
    if (!match && str.match(/^class[\s]/)) {
        ...
        if (!match) {
            ...
            match = str.match(/^(?:async[\s\w]+)?\(([\s\w,]*?)\)[\s]*=>/);
        }

        if (!match) {
            // ES6 arrow functions without parenthesis
            match = str.match(/^(?:async[\s\w]+)?([^=\s]*)[\s]*=>/);
...
function newInstance(ClassFn, args) {
    return new (Function.prototype.bind.apply(ClassFn, [null].concat(args)))();
}
